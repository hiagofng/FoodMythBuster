-- FoodMythBuster — exploratory queries on staging.stg_products
...
-- 2. Health-claim products by NOVA group — how many are ultra-processed?
SELECT
    nova_group,
    count(*)                                                        AS with_health_claim,
...
SELECT
    ...
    nova_group,
    additives_count,
    matched_health_claims
FROM staging.stg_products
WHERE is_nutriscore_a_but_nova4
ORDER BY additives_count DESC
LIMIT 20;

-- 4. Most deceptive brands (min 5 products, ranked by % deceptive)
SELECT
    brands,
    count(*)                                                        AS total_products,
    sum(is_deceptive::INT)                                          AS deceptive_count,
...
-- 5. Most common health claims found on NOVA 4 products
SELECT
